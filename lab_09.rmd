...
md_voting <- md_votes_by_type %>% 
  group_by(Year) %>%
  summarize(
    early_voting = sum(Early)
    election_day = sum("Election Day")
    mail_voting = sum(Mail),
    md_votes = early_voting + election_day + mail_voting
  ) %>%
  mutate(
...
```{r}
 totals_young <- read_csv("data/ev_totals_18_24.csv")
```
