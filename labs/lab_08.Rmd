...
md_absentee_ballots <- md_absentee_ballots %>% 
  mutate (percent_total_received = (total_received/total_sent)*100) %>% 
  mutate (dem_total_received = (dem_received/dem_sent)*100) 
...
md_absentee_ballots %>% 
  ggplot() +
  geom_bar(aes(x=reorder(county_name, percent_total_received), weight=percent_total_received)) +
  coord_flip() + 
  theme_economist()
...
md_absentee_ballots %>% 
  ggplot() +
  geom_bar(aes(x=reorder(county_name, -dem_total_received), weight=dem_total_received)) +
  coord_flip() + 
  theme_economist()
```
