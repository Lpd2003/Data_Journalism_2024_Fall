...
**Task** Load the five CSV files. Do they have the same column structure? **The column structure is the same, but some of the column names are mixed up.**
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Down to just one row, should be a 24x9 table.**
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **There was a :left join between the two dataframes. All rows from maryland_population are retained and matched with rows from county_voters_2020**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Prince George's County. The county is over 78% democrat, about 61% of the 18+ population is Black.**
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Howard county. 19% of 18+ population is Asian, 52% of the county is democratic. Howard county is not as overwhelmingly democratic as Prince George's. Unclear whether or not the Asian population plays into the political landscape of Howard county.**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **dbl (double)**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **character (chr)**
