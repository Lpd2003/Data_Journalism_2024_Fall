...
**Task** Run the following code to load some campaign expenditure data focused on food-related expenses in Washington, D.C. **As the son of D.C. small business owners, I think it's cool that many local businesses impact global politics.**
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Cleaned up W. Millar and Co. catering, Walter's Sports Bar down to two variations instead of three, WATRGATERETAILGARAGE60351 not found on the second.**
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **There are changes, however I wouldn't say that the data varies substantially. There are still a lot of similarities**
