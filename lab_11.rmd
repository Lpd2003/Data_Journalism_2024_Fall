...
```{r}
Frederick_Results <-"https://elections.maryland.gov/elections/2024/general_Results/gen_results_2024_by_county_11.html"
frederick_web <- read_html(Frederick_Results)

tables <- frederick_web %>% html_nodes("table")

tables[[9]] %>% html_table(fill = TRUE) %>% head()

frederick_board <- tables[[9]] %>% html_table(fill = TRUE)

colnames(frederick_board) <- c("Name", "Early Voting", "Election Day", "Mail In Ballot", "Provisional", "Total", "Percentage")

frederick_board$Percentage <- as.numeric(gsub("%", "", frederick_board$Percentage))
