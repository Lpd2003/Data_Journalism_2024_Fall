...
**https://datawrapper.dwcdn.net/8sw6g/1/ The story here is how much of an outlier 2020 is compared to other elections. Mail-In voting was at an all-time high, election day voting was very low, and more people voted early. 2022 saw a pretty significant bounceback to the norm.**

```{r}
md_votes_by_type <- read_csv("data/md_votes_by_type.csv") %>% 
  clean_names()
md_voting <- md_votes_by_type %>% 
  mutate(
...
write_csv(md_voting,"data/md_voting.csv")
...
**https://datawrapper.dwcdn.net/AKUJU/1/ I chose voters 18-24 because I was interested in how young voters show up in early voting.** 

```{r}
 young_voters <- read_csv("data/ev_totals_18_24.csv") %>% 
  clean_names() %>% 
  select(day, montgomery, frederick, prince_georges, howard) %>% 
  mutate(total = montgomery + frederick + prince_georges + howard)


  write_csv(young_voters, "data/young_voters.csv")
