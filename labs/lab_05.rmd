...
colnames(july_harris)
colnames(august_harris)
colnames(new_voters)

harris_donations <- bind_rows(july_harris, august_harris)
harris_new_voters <- bind_rows(harris_donations, new_voters)

print(colnames(harris_donations))
head(harris_donations)

new_voters1 <- new_voters %>%
  mutate(FullAddress = paste(HouseNumber, StreetName, StreetType, StreetType, ResidentialCity, ResidentialState, ResidentialZip, sep = ", "))
harris_donations1 <- harris_donations %>%
  mutate(FullAddress = paste(address_one, city, state, zip, sep = ", ")) 

print(colnames(harris_donations))

harris_donations1 <- harris_donations %>%
  mutate(FullName = paste(july_harris, august_harris))

new_voters1 <- new_voters %>%
  mutate(FullName = paste(FirstName, LastName))

joined_data <- harris_donations %>%
  inner_join(new_voters, by = c("FullName" = "FullName", "FullAddress" = "FullAddress"))
...
I chose to join the most relevant columns, excluding data such as "HouseSuffix" and "StreetPreDirection".
